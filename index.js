...
colors.setTheme({info: 'grey', warn: 'yellow', error: 'red', success: 'green'});
var algo = require("./mySmartAlgo");

var myGuessTracker = {};

if(!config.team || config.team.trim() === ""){
    console.log(`Please put your own team name and password into config.js and start again`.error);
    return;
}
...
function gameStatusReceived(err, data){
    if(err){
        ...
        console.log('');
        console.log(`GameId:${data.gameId} RoundId:${data.roundId} Status:${data.status}`.info);
        console.log('---------------------------------------------------------'.info);

        var key = `${data.gameId}-${data.roundId}`;
        //var haveIJoined = _.find(data.participants, function(o) { return o.name === config.team.toUpperCase() && o.joinedInThisRound === true; }) != null;
        var haveIJoined = _.find(data.participants, function(o) { return o.teamId === config.team; }) != null;
        switch(data.status){
            case 'Joining':
                //joining phase
                myGuessTracker = {};
            ...
            case 'Running':
                if(!haveIJoined){
                    ...
                    var amIAlive = _.find(data.participants, function(o) { return o.teamId === config.team && o.isAlive === true;}) != null;
                    if(!amIAlive){
